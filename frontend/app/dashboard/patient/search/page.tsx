...
import { Search, Star, Clock, Zap, Hash, Layers, Sparkles } from "lucide-react";

interface SearchResult {
  chunk_text: string;
  source_document: string;
  page_or_chunk_number: number;
...
export default function SearchPage() {
  ...
  async function doSearch() {
    ...
    setResults([]);
    try {
      const res = await apiFetch<{ results: SearchResult[]; search_id: string; response_time_ms: number }>("/search/hybrid", {
        method: "POST",
  ...
  async function rateSearch(search_id: string, rating: number) {
    try {
      await apiFetch("/search/rate", {
        method: "POST",
        body: JSON.stringify({ search_id, rating }),
      });
  ...
  return (
    <div className="space-y-8 max-w-4xl">
      ...
      >
        ...
        <div className="pointer-events-none absolute -bottom-20 -left-12 h-64 w-64 rounded-full bg-cyan-500/20 blur-3xl" />
        <div className="relative flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex items-center gap-4">
            <motion.div variants={fadeUp}
              className="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-teal-400 via-cyan-500 to-sky-500 shadow-glow-cyan">
              <Search className="h-7 w-7 text-white" strokeWidth={2.2} />
            </motion.div>
            <div>
              <motion.h2 variants={fadeUp} className="text-2xl font-bold tracking-tight text-ink-50 sm:text-3xl">
                Smart <span className="gradient-text-animated">Search</span>
              </motion.h2>
              <motion.p variants={fadeUp} className="mt-1 text-sm text-ink-200">
                Hybrid retrieval across your medical records — keyword + semantic.
              </motion.p>
            </div>
          </div>
          <motion.div variants={fadeUp} className="flex items-center gap-2">
            <span className="badge-cyan"><Sparkles className="h-3.5 w-3.5" /> Hybrid AI</span>
          </motion.div>
        </div>
      </motion.div>
        ...
        {(["search", "history"] as const).map(tab => (
          <button
            ...
            className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-all capitalize
              ${activeTab === tab ? "bg-teal-500/20 text-teal-300 border border-teal-500/30" : "text-gray-500 hover:text-gray-300"}`}
          >
            {tab}
          </button>
        ))}
          ...
          >
            ...
            {responseTime !== null && (
              <div className="flex items-center gap-3 text-xs text-gray-500">
                ...
                <span className="text-gray-600">Hybrid: 40% keyword + 60% semantic</span>
              </div>
            )}
              ...
              >
                  ...
                  >
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                      <div className="flex items-center gap-2">
                        <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${MATCH_COLORS[r.match_type] ?? MATCH_COLORS.hybrid}`}>
                          {r.match_type}
                        </span>
                        <span className="text-xs text-gray-500 flex items-center gap-1">
                          <Hash className="w-3 h-3" />Chunk {r.page_or_chunk_number}
                        </span>
                      </div>
                ...
                {searchId && (
                  <motion.div variants={fadeUp} className="glass-sm p-4 flex items-center gap-3">
                    <span className="text-sm text-gray-400">Rate these results:</span>
                    <div className="flex gap-1">
                      {[1, 2, 3, 4, 5].map(star => (
                        <motion.button
                          key={star}
                    ...
                    <tbody className="divide-y divide-gray-800/50">
                      {history.map(s => (
                        <tr key={s.id} className="hover:bg-gray-800/20 transition-colors">
                          <td className="px-6 py-3 text-gray-300 max-w-[200px] truncate">{s.query_text}</td>
                          <td className="px-4 py-3">
                            <span className={`text-[10px] font-semibold px-2 py-0.5 rounded-full border ${MATCH_COLORS[s.search_type] ?? MATCH_COLORS.hybrid}`}>
                              {s.search_type}
                            </span>
                          </td>
